...
class SpanningTree {
    ...
    visit(parent, child, distance) {
        ...
        if (newChildDist < currentChildDist) {
            ...
            // const distanceToEnd = end % width - child % width;  // Partial Heuristic: Only distance in x
            const distanceToEnd = graph.nodeCoords[child].dist(graph.nodeCoords[end]) // Complete heuristic: euclidean distance
            this.candidates.enqueue({node: child, dist: newChildDist + distanceToEnd}) 
        }
    }
